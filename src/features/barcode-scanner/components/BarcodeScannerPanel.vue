...
const historyForDisplay = computed(() => [...history.value].reverse())
...
<template>
  <section class="grid gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
    <header class="grid gap-3">
      ...
      <p v-if="!hasBarcodeSupport" class="rounded-lg border border-amber-200 bg-amber-50 p-2 text-sm text-amber-700">
    ...
    <div class="flex flex-wrap items-center gap-2">
      <button
        ...
        type="button" :disabled="isScanning || !hasBarcodeSupport" @click="startScanning">
        {{ startButtonText }}
      </button>
      ...
      <button
        ...
        type="button" :disabled="!isCameraActive" @click="stopCamera()">
        Остановить камеру
      </button>

      <label class="ml-auto flex items-center gap-2 text-sm text-slate-700">
        <input v-model="autoPauseAfterFirstScan"
          class="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" type="checkbox" />
        Автопауза после первого чтения
      </label>
    </div>

    <video ref="videoRef" class="max-h-105 w-full rounded-xl bg-black object-cover shadow" autoplay playsinline muted />
    ...
    <section class="grid gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
      <div class="flex items-center justify-between">
        ...
        <button
          ...
          type="button" @click="resetDebugStats">
          Сбросить метрики
        </button>
      </div>
    ...
    <section class="grid gap-2 rounded-xl border border-slate-200 bg-white p-3">
      ...
      <pre
        class="overflow-auto rounded-md bg-slate-900 p-3 text-xs leading-5 text-slate-100"><code>{{ debugJson }}</code></pre>
    </section>
    ...
    <section class="grid gap-2">
      <div class="flex items-center justify-between">
        ...
        <button
          ...
          type="button" :disabled="history.length === 0" @click="clearHistory">
          Очистить
        </button>
      </div>
      ...
      <ul v-else class="max-h-60 space-y-2 overflow-auto rounded-lg border border-slate-200 bg-white p-2">
        <li v-for="record in historyForDisplay" :key="record.id"
